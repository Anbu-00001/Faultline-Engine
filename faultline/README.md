...
- **Fails closed on a stale index** — an empty blast radius only means "safe" if the graph was actually there. Faultline first reads Orbit's free `status` + `graph_status` and, if the project's code index is absent, partial, or still indexing (`projects.indexed < total_known`, or `source_code.Definition == 0`), it refuses to show a green ✅ — it returns a 🟡 *"can't vouch"* verdict that blocks when gating is on (advisory otherwise; `faultline-override` is the audited escape). Same instinct as Orbit's own indexer, which refuses stale cleanup on a degraded re-index.
